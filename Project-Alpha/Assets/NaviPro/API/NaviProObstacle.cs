...
[AddComponentMenu("NaviPro/NaviPro Obstacle")]
[DefaultExecutionOrder(-3)]
public class NaviProObstacle : MonoBehaviour
{
	...
	[Tooltip("The height offset above and below the given polygon where the Obstacle affects the Walkable Surface.\nThis should be set large enough to encompass the surface to be affected, but not so large it will hit other surfaces.")]
	[Min(0.0f)]
	public float Height = 2.0f;

	
	internal NaviProSector sector;
	internal NaviProNative.Obstacle obstacle;

	internal LinkedListNode<NaviProObstacle> coreref = null;
	bool created = false;
	...
	void OnEnable()
	{
		NaviProCore.Instance.insertObstacles.Add(this);
	}
	void OnDisable()
	{
		...
		if (NaviProCore.IsDestroyed())
			return;
		if (created)
			NaviProCore.Instance.removeObstacles.Add(this);
		else
			NaviProCore.Instance.insertObstacles.Remove(this);
	}

	// After changing the position or any of the points of this obstacle, this function can be called to effect the changes.
	public void Apply()
	{
		NaviProCore.Instance.insertObstacles.Add(this);
		NaviProCore.Instance.removeObstacles.Add(this);
	...
	internal bool create()
	{
		...
		obstacle = NaviProCore.System.CreateObstacle(getSector().sector, getTransformedPoints(), getPositionInSector(), getLocalUpVectorInSector(), Mathf.Abs(Height));
		getSector().obstacles.Add(this);
	...
	internal void destroy()
	{
		...
		NaviProCore.System.RemoveObstacle(getSector().sector, obstacle);
		getSector().obstacles.Remove(this);
		obstacle = null;
		sector = null;
...
#if UNITY_EDITOR
[CustomEditor(typeof(NaviProObstacle))]
public class NaviProObstacleEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		NaviProObstacle obj = (NaviProObstacle)target;
